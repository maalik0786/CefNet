...
namespace CefNet.Internal
{
	public sealed class OffscreenGraphics
	{
		...
		public void Render(DrawingContext drawingContext)
		{
			lock (_syncRoot)
			{
				if (ViewPixels != null)
				{
					...
					drawingContext.DrawImage(surface, 1, new Rect(0, 0, pixelSize.Width, pixelSize.Height),
						new Rect(surface.Size));
		...
		private WriteableBitmap GetSurface(PixelBuffer pixelBuffer)
		{
			...
			{
				surface?.Dispose();
				surface = new WriteableBitmap(new PixelSize(pixelBuffer.Width, pixelBuffer.Height), DpiScale.Dpi,
					PixelFormat.Bgra8888);
				pixelBuffer.Surface = surface;
			}

			using (var frameBuffer = surface.Lock())
			{
