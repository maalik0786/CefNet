...
namespace CefNet.Avalonia
{
	public partial class WebView : TemplatedControl, IAvaloniaWebViewPrivate, IDisposable
	{
		...
		/// </summary>
		public static readonly RoutedEvent<StartDraggingEventArgs> StartDraggingEvent =
			RoutedEvent.Register<WebView, StartDraggingEventArgs>(nameof(StartDragging), RoutingStrategies.Bubble);

		/// <summary>
		///  Identifies the <see cref="TextFound" /> routed event.
		/// </summary>
		public static readonly RoutedEvent TextFoundEvent =
			RoutedEvent.Register<WebView, TextFoundRoutedEventArgs>(nameof(TextFound), RoutingStrategies.Bubble);

		/// <summary>
		///  Identifies the <see cref="PdfPrintFinished" /> routed event.
		/// </summary>
		public static readonly RoutedEvent PdfPrintFinishedEvent =
			RoutedEvent.Register<WebView, PdfPrintFinishedRoutedEventArgs>(nameof(PdfPrintFinished),
				RoutingStrategies.Bubble);

		/// <summary>
		///  Identifies the <see cref="StatusTextChanged" /> routed event.
		/// </summary>
		public static readonly RoutedEvent StatusTextChangedEvent =
			RoutedEvent.Register<WebView, RoutedEventArgs>(nameof(StatusTextChanged), RoutingStrategies.Bubble);

		/// <summary>
		///  Identifies the ToolTip property.
		/// </summary>
		public static readonly StyledProperty<object> ToolTipProperty =
			AvaloniaProperty.Register<WebView, object>("ToolTip");
		...
		private bool _suppressLostFocusEvent;
		private CefRect _windowBounds;

		private Dictionary<InitialPropertyKeys, object> InitialPropertyBag =
			new Dictionary<InitialPropertyKeys, object>();
		...
		public WebView(WebView opener)
		{
			...
			AddHandler(KeyDownEvent, HandlePreviewKeyDown, RoutingStrategies.Tunnel, true);
			AddHandler(KeyUpEvent, HandlePreviewKeyUp, RoutingStrategies.Tunnel, true);
		...
		{
			var ppt = new PixelPoint(point.X, point.Y);
			...
			CefMenuModel model, CefRunContextMenuCallback callback)
		{
			...
			try
			{
				...
				return Dispatcher.UIThread.InvokeAsync(() => RunContextMenu(runner, pt)).GetAwaiter().GetResult();
			}
			finally
			{
		...
		/// <param name="handler">The handler.</param>
		public static void AddPdfPrintFinishedHandler(IInteractive element,
			EventHandler<PdfPrintFinishedRoutedEventArgs> handler)
		{
		...
		/// <param name="element">An object that raise the <see cref="PdfPrintFinished" /> routed event.</param>
		/// <param name="handler">The handler.</param>
		public static void RemovePdfPrintFinishedHandler(IInteractive element,
			EventHandler<PdfPrintFinishedRoutedEventArgs> handler)
		{
		...
		public override void Render(DrawingContext drawingContext)
		{
			...
			if (OffscreenGraphics != null)
				...
				drawingContext.DrawText(Brushes.Black, new Point(10, 10),
					new FormattedText
					{
						Text = GetType().Name, Typeface = new Typeface(FontFamily, FontSize, FontStyle, FontWeight)
					});
		}
		...
		protected internal virtual unsafe void OnUpdateRootBounds()
		{
			...
			{
				RECT windowBounds;
				var scale = OffscreenGraphics.DpiScale;

				if (PlatformInfo.IsWindows && NativeMethods.DwmIsCompositionEnabled()
				                           && NativeMethods.DwmGetWindowAttribute(window.PlatformImpl.Handle.Handle,
					                           DWMWINDOWATTRIBUTE.ExtendedFrameBounds, &windowBounds, sizeof(RECT)) ==
				                           0)
				{
		...
		protected internal void RootBoundsChanged(CefRect bounds)
		{
			...
			if (_allowResizeNotifications) NotifyRootMovedOrResized();
		}
		...
		public PixelPoint PointToScreen(Point point)
		{
			...
			{
				var pixelPoint = VisualExtensions.PointToScreen(this, point);
				var dpi = OffscreenGraphics.DpiScale;
				if (dpi.DpiScaleX == 1.0 && dpi.DpiScaleY == 1.0)
					return pixelPoint;
				return new PixelPoint((int) Math.Ceiling(pixelPoint.X / dpi.DpiScaleX),
					(int) Math.Ceiling(pixelPoint.Y / dpi.DpiScaleY));
			}

			var viewRect = OffscreenGraphics.GetBounds();
		...
		protected virtual async void OnStartDragging(StartDraggingEventArgs e)
		{
			...
			await DragDrop.DoDragDrop(_lastPointerPressedEventArgs, new CefNetDragData(this, e.Data),
				e.AllowedEffects.ToDragDropEffects());
		...
		protected override void OnPointerMoved(PointerEventArgs e)
		{
			...
			var pp = pointerPoint.Properties;
			if (pp.IsLeftButtonPressed)
				modifiers |= CefEventFlags.LeftMouseButton;
			if (pp.IsRightButtonPressed)
				modifiers |= CefEventFlags.RightMouseButton;
			var mousePos = pointerPoint.Position; // e.GetPosition(this);
		...
		protected override void OnPointerPressed(PointerPressedEventArgs e)
		{
			...
			if (e.GetCurrentPoint(null).Properties.PointerUpdateKind.GetMouseButton() <= MouseButton.Right)
			{
				...
				SendMouseDownEvent((int) mousePos.X, (int) mousePos.Y, GetButton(e), e.ClickCount,
		...
		protected override void OnPointerWheelChanged(PointerWheelEventArgs e)
		{
			...
			var mousePos = e.GetPosition(this);
			SendMouseWheelEvent((int) mousePos.X, (int) mousePos.Y, (int) e.Delta.X * WHEEL_DELTA,
				(int) e.Delta.Y * WHEEL_DELTA);
			e.Handled = true;
		}
		...
		protected virtual bool ProcessPreviewKey(CefKeyEventType eventType, KeyEventArgs e)
		{
			...
			var isSystemKey = e.KeyModifiers.HasFlag(KeyModifiers.Alt) || key == Key.LeftAlt || key == Key.RightAlt;
		...
		{
			var mousePos = e.GetPosition(this);
			SendDragEnterEvent((int) mousePos.X, (int) mousePos.Y, e.GetModifiers(), e.GetCefDragData(),
				e.DragEffects.ToCefDragOperationsMask());
		...
		{
			var mousePos = e.GetPosition(this);
			SendDragOverEvent((int) mousePos.X, (int) mousePos.Y, e.GetModifiers(),
				e.DragEffects.ToCefDragOperationsMask());
		...
		protected virtual void OnDrop(DragEventArgs e)
		{
			...
			{
				var data = (CefNetDragData) e.Data.Get(nameof(CefNetDragData));
		...
		{
			var modifiers = GetModifierKeys(e.KeyModifiers);
