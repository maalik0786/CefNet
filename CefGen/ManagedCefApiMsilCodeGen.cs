...
namespace CefGen
{
	internal sealed class ManagedCefApiMsilCodeGen : CodeGenBase
	{
		...
		public void GenerateFrom(ManagedCefApiTypes managedTypes)
		{
			var license = string.Join("\r\n",
				File.ReadAllText(Path.Combine("Settings", "LicenseTemplate.txt"), Encoding.UTF8).Trim().Split('\n')
					.Select(s => "// " + s.Trim()));
			foreach (var symbol in managedTypes.RefCountedClasses)
			{
				if (!symbol.AllInterfaces.Any(t => t.TypeKind == TypeKind.Interface && t.Name.EndsWith("Private")))
					continue;

				var avoids = symbol.GetMembers().OfType<IMethodSymbol>()
					.Where(m => m.Name.Split('.').Last().StartsWith("Avoid")).ToArray();
				if (avoids.Length == 0)
					continue;
				...
				try
				{
					Output.WriteLine(license, nameof(CefGen),
						filePath.Substring(filePath.IndexOf("Generated")).Replace('\\', '/'));
					Output.WriteLine();
		...
		{
			var token = target.ToDisplayString();
			var pos = token.IndexOf('(');
			token = TypeToMsil(target.ReturnType) + " " + NameToMsil(token.Remove(pos)) + "(" +
			        string.Join(", ", target.Parameters.Select(ParamToMsil)) + ")";
		...
		private static string TypeToMsil(ITypeSymbol symbol)
		{
			...
			if (symbol.IsValueType) return "valuetype " + symbol;
			return "class " + symbol;
		}

		private IMethodSymbol FindTarget(IMethodSymbol func, INamedTypeSymbol type)
		{
			var fnName = func.Name.Substring(func.Name.LastIndexOf('.') + 1);
