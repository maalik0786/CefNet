...
{
	internal sealed class CustomWebViewGlue :
#if MODERNFORMS
...
#endif
	{
		...
		private readonly Dictionary<Guid, CefResourceHandler> _customSources =
			new Dictionary<Guid, CefResourceHandler>();
		...
		private new CustomWebView WebView => (CustomWebView) base.WebView;

		protected override void OnBeforeContextMenu(CefBrowser browser, CefFrame frame, CefContextMenuParams menuParams,
			CefMenuModel model)
		{
			if (model.Count > 0)
				model.AddSeparator();
			...
			var submenu = model.AddSubMenu(0, "Submenu Test");
		...
		protected override bool OnContextMenuCommand(CefBrowser browser, CefFrame frame,
		...
		{
			if (Guid.TryParse(request.GetHeaderByName("CefNet-Source"), out var sourceKey)
			    && _customSources.Remove(sourceKey, out var resourceHandler))
