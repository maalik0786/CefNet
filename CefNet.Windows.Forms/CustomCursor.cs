	...
	public sealed class CustomCursor
	{
		...
		private static Cursor CreateTransparent()
		{
			...
			try
			{
				...
				if (!NativeMethods.GetIconInfo(iconHandle, out var iconInfo))
		...
		public static Cursor Create(ref CefCursorInfo cursorInfo)
		{
			...
			try
			{
				...
				{
					bitmap = new Bitmap(size.Width, size.Height, 4 * size.Width, PixelFormat.Format32bppArgb,
						cursorInfo.Buffer);
					iconHandle = bitmap.GetHicon();
					if (iconHandle != IntPtr.Zero && NativeMethods.GetIconInfo(iconHandle, out var iconInfo))
					{
		...
		~CustomCursor()
		{
			if (Interlocked.Exchange(ref _cursor, null) != null) NativeMethods.DestroyIcon(_cursorHandle);
		}
	}
}