...
namespace CefNet.Internal
{
	public partial class WebViewGlue
	{
		...
		/// </param>
		/// <returns></returns>
		protected internal virtual bool OnBeforePopup(CefBrowser browser, CefFrame frame, string targetUrl,
			string targetFrameName, CefWindowOpenDisposition targetDisposition,
			bool userGesture, CefPopupFeatures popupFeatures, CefWindowInfo windowInfo, ref CefClient client,
			CefBrowserSettings settings, ref CefDictionaryValue extraInfo, ref int noJavascriptAccess)
		{
#if DEBUG
			if (!BrowserObject.IsSame(browser))
				throw new InvalidOperationException();
#endif
			var ea = new CreateWindowEventArgs(frame, targetUrl, targetFrameName, targetDisposition, userGesture,
				popupFeatures, windowInfo, null, settings, extraInfo, noJavascriptAccess != 0);
			WebView.RaiseCefCreateWindow(ea);
			extraInfo = ea.ExtraInfo;
			noJavascriptAccess = ea.NoJavaScriptAccess ? 1 : 0;
			client = ea.Client;
			if (!ea.Cancel) (client as CefClientGlue)?.NotifyPopupBrowserCreating();
			return ea.Cancel;
		}
		...
		protected internal virtual void OnAfterCreated(CefBrowser browser)
		{
#if DEBUG
		...
		/// </summary>
		/// <param name="browser">The browser instance.</param>
		protected internal virtual bool DoClose(CefBrowser browser)
		{
#if DEBUG
			if (!BrowserObject.IsSame(browser))
				throw new InvalidOperationException();
		...
		/// </remarks>
		protected internal virtual void OnBeforeClose(CefBrowser browser)
		{
#if DEBUG
			if (!BrowserObject.IsSame(browser))
				throw new InvalidOperationException();
