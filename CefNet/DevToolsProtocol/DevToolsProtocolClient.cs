...
namespace CefNet
{
	internal sealed class DevToolsProtocolClient : CefDevToolsMessageObserver
	{
		...
		private readonly Dictionary<int, TaskCompletionSource<DevToolsMethodResult>> _waitTasks =
			new Dictionary<int, TaskCompletionSource<DevToolsMethodResult>>();
		...
		private object SyncRoot => _waitTasks;
		...
		{
			int id;
			lock (SyncRoot) id = ++_lastMessageId;
		...
		public Task<DevToolsMethodResult> WaitForMessageAsync(int id) => RemoveOrAddTaskSource(id).Task;
...
//#else
					//m = new TaskCompletionSource<DevToolsMethodResult>(TaskCreationOptions
...
//#endif
		...
		protected internal override void OnDevToolsEvent(CefBrowser browser, string method, IntPtr @params,
			long paramsSize)
		{
			string args;
			...
			{
//#if NET45
				var buffer = new byte[paramsSize];
...
//#endif
		...
		protected internal override void OnDevToolsMethodResult(CefBrowser browser, int messageId, bool success,
			IntPtr result, long resultSize)
		{
			byte[] buffer;
...
//#if NET45
		...
		{
			var r = (DevToolsMethodResult) state;
