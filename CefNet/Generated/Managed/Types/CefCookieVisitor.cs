...
namespace CefNet
{
	...
	public unsafe partial class CefCookieVisitor : CefBaseRefCounted<cef_cookie_visitor_t>, ICefCookieVisitorPrivate
	{
		...
		internal static CefCookieVisitor Create(IntPtr instance)
		{
		...
		/// </summary>
		protected internal virtual bool Visit(CefCookie cookie, int count, int total, ref int deleteCookie)
		{
			return default;
		}

		// int (*)(_cef_cookie_visitor_t* self, const const _cef_cookie_t* cookie, int count, int total, int* deleteCookie)*
		private static int VisitImpl(cef_cookie_visitor_t* self, cef_cookie_t* cookie, int count, int total,
			int* deleteCookie)
		{
			var instance = GetInstance((IntPtr) self) as CefCookieVisitor;
			if (instance == null || ((ICefCookieVisitorPrivate) instance).AvoidVisit()) return default;
			return instance.Visit(*(CefCookie*) cookie, count, total, ref *deleteCookie) ? 1 : 0;
		}

		[UnmanagedFunctionPointer(CallingConvention.Winapi)]
		private delegate int VisitDelegate(cef_cookie_visitor_t* self, cef_cookie_t* cookie, int count, int total,
			int* deleteCookie);
	}
}