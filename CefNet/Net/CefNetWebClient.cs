...
namespace CefNet.Net
{
	...
	public class CefNetWebClient : ICefNetCredentialProvider
	{
		...
		/// </remarks>
		public virtual Task<NetworkCredential> GetCredentialAsync(bool proxy, string host, int port, string realm,
		...
		public async Task<CefNetWebRequest> GetAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			NameValueCollection headers, CancellationToken cancellationToken)
		{
			if (requestUri is null)
				throw new ArgumentNullException(nameof(requestUri));

			var r = new CefRequest();
			r.Flags = (int) RequestFlags;
		...
		/// <returns>The task object representing the asynchronous operation.</returns>
		/// <exception cref="ArgumentNullException">The <see cref="requestUri" /> parameter is null.</exception>
		public Task<CefNetWebRequest> GetAsync(string requestUri, string referrerUri, CefReferrerPolicy referrerPolicy,
			NameValueCollection headers, CancellationToken cancellationToken)
		{
			return GetAsync(new Uri(requestUri, UriKind.Absolute),
				referrerUri is null ? null : new Uri(referrerUri, UriKind.Absolute), referrerPolicy, headers,
				cancellationToken);
		}
		...
		/// <returns>The task object representing the asynchronous operation.</returns>
		/// <exception cref="ArgumentNullException">The <see cref="requestUri" /> parameter is null.</exception>
		public Task<CefNetWebRequest> GetAsync(Uri requestUri, NameValueCollection headers,
		...
		/// <returns>The task object representing the asynchronous operation.</returns>
		/// <exception cref="ArgumentNullException">The <see cref="requestUri" /> parameter is null.</exception>
		public Task<CefNetWebRequest> GetAsync(string requestUri, NameValueCollection headers,
			CancellationToken cancellationToken)
		{
			return GetAsync(new Uri(requestUri, UriKind.Absolute), null, CefReferrerPolicy.Default, headers,
				cancellationToken);
		}
		...
		{
			return GetAsync(new Uri(requestUri, UriKind.Absolute), null, CefReferrerPolicy.Default, null,
				cancellationToken);
		}
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<string> GetStringAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			NameValueCollection headers, CancellationToken cancellationToken)
		{
			var request = await GetAsync(requestUri, referrerUri, referrerPolicy, headers, cancellationToken)
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<Stream> GetStreamAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			NameValueCollection headers, CancellationToken cancellationToken)
		{
			if (requestUri is null)
				throw new ArgumentNullException(nameof(requestUri));
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<byte[]> GetDataAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			NameValueCollection headers, CancellationToken cancellationToken)
		{
			using var stream =
				await GetStreamAsync(requestUri, referrerUri, referrerPolicy, headers, cancellationToken)
					.ConfigureAwait(false);
			if (stream is MemoryStream mem && mem.Capacity == mem.Length) return mem.GetBuffer();
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<CefNetWebRequest> PostAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			CefPostData content, NameValueCollection headers, CancellationToken cancellationToken)
		{
			if (requestUri is null)
				throw new ArgumentNullException(nameof(requestUri));
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public Task<CefNetWebRequest> PostAsync(Uri requestUri, CefPostData content, NameValueCollection headers,
			CancellationToken cancellationToken)
		{
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public Task<CefNetWebRequest> PostAsync(Uri requestUri, CefPostData content,
			CancellationToken cancellationToken)
		{
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<CefNetWebRequest> PostAsync(Uri requestUri, NameValueCollection content,
			CancellationToken cancellationToken)
		{
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<CefNetWebRequest> PostAsync(Uri requestUri, IEnumerable<KeyValuePair<string, string>> content,
			CancellationToken cancellationToken)
		{
			using (var postData = new CefPostData(content))
			{
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<string> PostReadStringAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			CefPostData content, NameValueCollection headers, CancellationToken cancellationToken)
		{
			var request = await PostAsync(requestUri, referrerUri, referrerPolicy, content, headers, cancellationToken)
				.ConfigureAwait(false);
			AssertSuccess(request);
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<string> PostReadStringAsync(Uri requestUri, NameValueCollection content,
			CancellationToken cancellationToken)
		{
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<string> PostReadStringAsync(Uri requestUri, IEnumerable<KeyValuePair<string, string>> content,
			CancellationToken cancellationToken)
		{
			var request = await PostAsync(requestUri, content, cancellationToken).ConfigureAwait(false);
			AssertSuccess(request);
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<Stream> PostReadStreamAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			CefPostData content, NameValueCollection headers, CancellationToken cancellationToken)
		{
			var request = await PostAsync(requestUri, referrerUri, referrerPolicy, content, headers, cancellationToken)
				.ConfigureAwait(false);
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<Stream> PostReadStreamAsync(Uri requestUri, NameValueCollection content,
			CancellationToken cancellationToken)
		{
			var request = await PostAsync(requestUri, content, cancellationToken).ConfigureAwait(false);
			AssertSuccess(request);
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<Stream> PostReadStreamAsync(Uri requestUri, IEnumerable<KeyValuePair<string, string>> content,
			CancellationToken cancellationToken)
		{
			var request = await PostAsync(requestUri, content, cancellationToken).ConfigureAwait(false);
			AssertSuccess(request);
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<byte[]> PostReadDataAsync(Uri requestUri, Uri referrerUri, CefReferrerPolicy referrerPolicy,
			CefPostData content, NameValueCollection headers, CancellationToken cancellationToken)
		{
			using var stream =
				await PostReadStreamAsync(requestUri, referrerUri, referrerPolicy, content, headers, cancellationToken)
					.ConfigureAwait(false);
			if (stream is MemoryStream mem && mem.Capacity == mem.Length) return mem.GetBuffer();
			var buffer = new byte[stream.Length];
			if (buffer.Length != await stream.ReadAsync(buffer, 0, buffer.Length).ConfigureAwait(false))
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<byte[]> PostReadDataAsync(Uri requestUri, NameValueCollection content,
			CancellationToken cancellationToken)
		{
			using var stream = await PostReadStreamAsync(requestUri, content, cancellationToken).ConfigureAwait(false);
			if (stream is MemoryStream mem && mem.Capacity == mem.Length) return mem.GetBuffer();
			var buffer = new byte[stream.Length];
			if (buffer.Length != await stream.ReadAsync(buffer, 0, buffer.Length).ConfigureAwait(false))
				throw new InvalidOperationException();
		...
		/// </param>
		/// <returns>The task object representing the asynchronous operation.</returns>
		public async Task<byte[]> PostReadDataAsync(Uri requestUri, IEnumerable<KeyValuePair<string, string>> content,
			CancellationToken cancellationToken)
		{
			using var stream = await PostReadStreamAsync(requestUri, content, cancellationToken).ConfigureAwait(false);
			if (stream is MemoryStream mem && mem.Capacity == mem.Length) return mem.GetBuffer();
		...
		protected virtual void AssertSuccess(CefNetWebRequest request)
		{
			if (request.RequestStatus != CefUrlRequestStatus.Success)
				throw new WebException("Error: " + request.RequestError);
		}
	}
}