...
namespace CefNet
{
	...
	public static unsafe class CefString
	{
		...
		public static string ReadAndFree(cef_string_t* str)
		{
			...
			var rv = Marshal.PtrToStringUni((IntPtr) s->str, (int) s->length);
			if (s->dtor != default)
			{
				s->Dtor(s->str);
				s->dtor = default;
		...
		public static void Replace(cef_string_t* str, string value)
		{
			...
			{
				var s = (cef_string_utf16_t*) str;
				if (s->str != default && s->dtor != default) s->Dtor(s->str);
				if (value == null)
				{
					s->str = null;
					s->length = default;
				...
				else
				{
					...
					s->length = (UIntPtr) value.Length;
					s->dtor = DestructorAddress;
				}
			}
		}
