...
using CefNet.WinApi;
using KeyInterop = CefNet.Input.Linux.KeyInterop;
...
namespace CefNet.Input
{
	...
	public class KeycodeConverter
	{
		...
		public virtual VirtualKeys CharacterToVirtualKey(char character)
		{
			...
			{
				var virtualKeyCode = WinApi.NativeMethods.VkKeyScan(character) & 0xFF;
			...
			{
				var keysym = KeyInterop.CharToXKeySym(character);
				if (keysym == XKeySym.None)
					keysym = CefNet.Linux.NativeMethods.XStringToKeysym("U" + ((int) character).ToString("X"));
				return KeyInterop.XKeySymToVirtualKey(TranslateXKeySymToAsciiXKeySym(keysym));
			}
			...
			if (PlatformInfo.IsMacOS)
			{
				// US QWERTY only
				var keysym = KeyInterop.CharToXKeySym(character);
				return KeyInterop.XKeySymToVirtualKey(keysym);
			}
		...
		{
			var scan_code = NativeMethods.MapVirtualKey((uint) key, MapVirtualKeyType.MAPVK_VK_TO_VSC);
		...
		{
			var vkcode = MacOS.KeyInterop.WindowsKeyToMacOSKey(key, extended);
		...
		{
			var keysym = KeyInterop.VirtualKeyToXKeySym(key, shift);
			if (keysym == XKeySym.None)
				return 0;
		...
		{
			var virtualKey = WinApi.NativeMethods.VkKeyScan(character);
		...
		public static int GetMacOSNativeKeyCode(char character, bool extended)
		{
			...
			if (keysym == XKeySym.None)
				return 0;
			return GetMacOSNativeKeyCode(KeyInterop.XKeySymToVirtualKey(keysym), extended);
		}
		...
		{
			var display = CefNet.Linux.NativeMethods.XOpenDisplay(IntPtr.Zero);
			try
			{
		...
		{
			var value = (uint) lParam.ToPointer();
			var scan_code = (value >> 16) & 0x00FFU;
		...
		public static unsafe IntPtr GetLParamFromWindowsScanCode(CefKeyEventType eventType, int repeatCount,
			byte scanCode, bool isSystemKey, bool isExtended)
		{
			if (repeatCount < 0)
		...
		public static XKeySym TranslateXKeySymToAsciiXKeySym(XKeySym keysym)
		{
			...
			try
			{
				var keycode = CefNet.Linux.NativeMethods.XKeysymToKeycode(display, keysym);
		...
		/// <summary>
		///  Converts a combination of a virtual key code and a modifier key state into a string of one or more Unicode
		///  characters.
		/// </summary>
		...
		{
			var keysym = KeyInterop.VirtualKeyToXKeySym(virtualKey, modifiers.HasFlag(CefEventFlags.ShiftDown));
			if (keysym == XKeySym.None)
				return char.MinValue;
			return KeyInterop.XKeySymToChar(keysym);
		}
	}
}