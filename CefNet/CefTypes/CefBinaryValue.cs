...
namespace CefNet
{
	public unsafe partial class CefBinaryValue
	{
		...
		public CefBinaryValue(IntPtr data, int size)
			: this(CefNativeApi.cef_binary_value_create((void*) data,
				size >= 0 ? (UIntPtr) size : throw new ArgumentOutOfRangeException(nameof(size))))
		{
		}
		...
		private static cef_binary_value_t* CreateFromBuffer(byte[] buffer)
		{
			...
			{
				return CefNativeApi.cef_binary_value_create(data, (UIntPtr) buffer.Length);
			}
		}
