...
namespace CefNet
{
	public partial class CefFrame
	{
		...
		public unsafe CefFrame[] GetFrames()
		{
			...
			var ids = browser.GetFrameIdentifiers();
			var frames = new List<CefFrame>(ids.Length);
			foreach (var fid in ids)
			{
				var frame = browser.NativeInstance->GetFrameByIdent(fid);
				if (frame == null)
					continue;
				...
				if (parent != null)
				{
					var parentid = parent->GetIdentifier();
					((cef_base_ref_counted_t*) parent)->Release();
					if (parentid == frameid)
					{
						frames.Add(Wrap(f => new CefFrame((cef_frame_t*) f), frame));
						continue;
					}
				}
