	...
	public sealed unsafe class CefStringMap : IDisposable, IDictionary<string, string>
	{
		...
		public string this[int index]
		{
			get
			{
				...
				var s0 = new cef_string_t();
				var rv = CefNativeApi.cef_string_map_value(Instance, new UIntPtr((uint) index), &s0);
				var value = CefString.ReadAndFree(&s0);
				if (rv == 0)
					throw new ArgumentOutOfRangeException(nameof(index));
				return value;
			}
		...
		public string this[string key]
		{
			get
			{
				...
				fixed (char* k = key)
				{
					var s0 = new cef_string_t {Str = k, Length = key.Length};
					var s1 = new cef_string_t();
					var rv = CefNativeApi.cef_string_map_find(Instance, &s0, &s1);
					var value = CefString.ReadAndFree(&s1);
					if (rv == 0)
		...
		/// </summary>
		public int Count => (int) CefNativeApi.cef_string_map_size(Instance);
		...
		public void Add(string key, string value)
		{
			...
			{
				var s0 = new cef_string_t {Str = k, Length = key.Length};
				var s1 = new cef_string_t {Str = v, Length = value.Length};
		...
		public void Add(KeyValuePair<string, string> item)
		{
			...
			{
				var s0 = new cef_string_t {Str = k, Length = item.Key.Length};
				var s1 = new cef_string_t {Str = v, Length = item.Value.Length};
				CefNativeApi.cef_string_map_append(Instance, &s0, &s1);
			}
		}
		...
		public void CopyTo(KeyValuePair<string, string>[] array, int arrayIndex)
		{
			...
			for (uint i = 0; i < count; i++) array[arrayIndex + (int) i] = GetInternal(i);
		}

		/// <inheritdoc />
		public IEnumerator<KeyValuePair<string, string>> GetEnumerator()
		{
			var count = (uint) CefNativeApi.cef_string_map_size(Instance);
			for (uint i = 0; i < count; i++) yield return GetInternal(i);
		}
