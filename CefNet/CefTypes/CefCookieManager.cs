...
namespace CefNet
{
	public unsafe partial class CefCookieManager
	{
		...
		{
			return Wrap(Create,
				CefNativeApi.cef_cookie_manager_get_global_manager(callback != null
					? callback.GetNativeInstance()
					: null));
		}
		...
		/// <remarks>The result of the task can be null if cookies cannot be accessed.</remarks>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Task<CefNetCookie[]> GetCookiesAsync(string url, bool includeHttpOnly,
		...
		/// <returns>The task object representing the asynchronous operation.</returns>
		/// <remarks>The result of the task can be null if cookies cannot be accessed.</remarks>
		public Task<CefNetCookie[]> GetCookiesAsync(string url, bool includeHttpOnly, Func<CefCookie, bool> filter,
			CancellationToken cancellationToken)
		{
		...
		public bool SetCookie(string url, CefNetCookie cookie, CefSetCookieCallback callback)
		{
			if (url is null)
			...
			if (Uri.TryCreate(url, UriKind.Absolute, out var uri)
			    && (Uri.UriSchemeHttp.Equals(uri.Scheme, StringComparison.Ordinal) ||
			        Uri.UriSchemeHttps.Equals(uri.Scheme, StringComparison.Ordinal)))
			{
				var aCookie = cookie.ToCefCookie();
				try
				{
					...
					fixed (char* s0 = url)
					{
						...
						return SafeCall(NativeInstance->SetCookie(&cstr0, (cef_cookie_t*) &aCookie,
							callback != null ? callback.GetNativeInstance() : null) != 0);
					}
				}
				finally
				{
