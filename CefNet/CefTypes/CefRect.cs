...
namespace CefNet
{
	public partial struct CefRect : IEquatable<CefRect>
	{
		...
		public void Union(CefRect rect)
		{
			...
			var right = Math.Max(X + Width, rect.X + rect.Width);
			var y = Math.Min(Y, rect.Y);
			var bottom = Math.Max(Y + Height, rect.Y + rect.Height);
			_instance.x = x;
			_instance.y = y;
		...
		{
			var x = Math.Max(_instance.x, rect.X);
			var y = Math.Max(_instance.y, rect.Y);
			var right = Math.Min(_instance.x + _instance.width, rect.X + rect.Width);
			var bottom = Math.Min(_instance.y + _instance.height, rect.Y + rect.Height);
			if (x <= right && y <= bottom)
			{
				_instance.x = x;
				_instance.y = y;
		...
		public static CefRect Intersect(CefRect a, CefRect b)
		{
			...
			if (x <= right && y <= bottom) return new CefRect(x, y, right - x, bottom - y);
			return new CefRect();
		}
		...
		{
			if (rect.X < _instance.x + _instance.width && _instance.x < rect.X + rect.Width &&
			    rect.Y < _instance.y + _instance.height) return _instance.y < rect.Y + rect.Height;
			return false;
		}
		...
		{
			if (X <= pt.X && pt.X < X + Width && Y <= pt.Y) return pt.Y < Y + Height;
			return false;
		}
		...
		{
			if (X <= rect.X && rect.Right <= Right && Y <= rect.Y) return rect.Bottom <= Bottom;
			return false;
		}
