...
namespace CefNet
{
	public unsafe partial class CefPostData
	{
		...
		/// <summary>
		///  Adds the elements of the specified <see cref="NameValueCollection" /> as an application/x-www-form-urlencoded
		///  string.
		/// </summary>
		...
		public void AddUrlEncoded(NameValueCollection content)
		{
			...
			using (var w = new StreamWriter(ms, Encoding.ASCII, 1024, true))
			{
				...
				{
					var safeKey = key is null ? null : Uri.EscapeDataString(key);
					foreach (var value in content.GetValues(key))
					{
						w.Write(safeKey);
						if (value != null)
						{
