...
namespace CefNet
{
	public unsafe partial class CefThread
	{
		...
		/// </param>
		public CefThread(string name, CefThreadPriority priority, CefMessageLoopType messageLoopType, bool stoppable,
			CefComInitMode comInitMode)
			: this(CreateInternal(name, priority, messageLoopType, stoppable, comInitMode))
		{
		}

		private static cef_thread_t* CreateInternal(string name, CefThreadPriority priority,
			CefMessageLoopType messageLoopType, bool stoppable, CefComInitMode comInitMode)
		{
			if (name == null)
			...
			fixed (char* s0 = name)
			{
				...
				return CefNativeApi.cef_thread_create(&cstr0, priority, messageLoopType, stoppable ? 1 : 0,
					comInitMode);
			}
		}
	}
