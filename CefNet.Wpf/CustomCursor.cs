...
namespace CefNet.Wpf
{
	...
	{
		public static unsafe Cursor Create(ref CefCursorInfo cursorInfo)
		{
			var size = cursorInfo.Size;
			if (size.Width > 0 && size.Height > 0 && cursorInfo.Buffer != IntPtr.Zero)
				try
				{
					var bufferSize = size.Width * size.Height * 4;
					var ICON_HEADER_SIZE = sizeof(ICONDIR);
					var stream = new MemoryStream();
					{
						var dpi = OffscreenGraphics.DpiScale;
						var source = BitmapSource.Create(size.Width, size.Height, dpi.PixelsPerInchX,
							dpi.PixelsPerInchY, PixelFormats.Bgra32, null, cursorInfo.Buffer, bufferSize,
							size.Width << 2);
