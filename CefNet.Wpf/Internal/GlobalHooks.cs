...
namespace CefNet.Internal
{
	internal sealed class GlobalHooks
	{
		...
		private readonly HwndSourceHook _delegate;
		...
		internal static void Initialize(WebView view)
		{
			...
			EventManager.RegisterClassHandler(typeof(Window), FrameworkElement.SizeChangedEvent,
				new RoutedEventHandler(TryAddGlobalHook));

			foreach (Window window in Application.Current.Windows) TryAddGlobalHook(window);
		}
		...
		{
			var point = new Point(unchecked((int) lParam) & 0xFFFF, (unchecked((int) lParam) >> 16) & 0xFFFF);
			point = _source.RootVisual.PointFromScreen(point);
			_target = null;
			VisualTreeHelper.HitTest(_source.RootVisual, null, HitTestResultCallbackHandler,
			...
			try
			{
				...
				var ea = new MouseWheelEventArgs(InputManager.Current.PrimaryMouseDevice, Environment.TickCount,
					-(short) ((unchecked((int) wParam) >> 16) & 0xFFFF));
